...
gPRC Interceptor is a powerful mechanism to add additional logic such as logging, authentication, authorization, metrics, tracing, and any other customer requirements.

Here we only talk about server side unary interceptor. Client side interceptor and stream interceptor are very similar to the server side.
...
These is one of the key pints. Please remember it.
...
gRPC can prepare the interceptor chain when it's ready to create a gRPC server. in `NewServer()` function, `chainUnaryServerInterceptors(s)` will chain the interceptors together.
...
* `getChainUnaryHandler` return an anonymous `UnaryHandler` or the `finalHandler`. The anonymous wrapper ensure the function body will not be called at this moment.
...
gRPC will generate the following code to register the business service. Please note the **ServiceName**, **MethodName** and **Handler**. Your business service will not be used directly, the `Greeter_SayHello_Handler` will be used instead.
...
* Use `dec(in)` to decode the incoming message and construct the `HelloRequest` object.
...
* `df` is a anonymous function which will decode the gRPC request frame and build the parameter.
...
To use interceptor, you need to implement an interceptor and add it to the gRPC server. Although using it is simple, now you understand what happens under the hood. Can you image what happens when you call the `handler(ctx, req)`?
...
Add an interceptor to the server is also simple. Just add some `ServerOption` as parameters for `grpc.NewServer` function invocation.
...
Here `grpc.UnaryInterceptor(unaryInterceptor)` return `ServerOption`. Actually its return type is `funcServerOption`. An implementation of `ServerOption` interface.
